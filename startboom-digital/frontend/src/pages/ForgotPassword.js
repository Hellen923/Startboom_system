                ...
                <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="mx-auto w-full max-w-sm lg:max-w-md">
                    <div className="text-center lg:text-left mb-10">
                        ...
                        <p className="text-gray-600 dark:text-gray-400">
                            {emailSent ? "We've sent a 6-digit code to your email." : "Enter your email address to receive a reset code."}
                        </p>
                    </div>

                    {emailSent ? (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-green-800 dark:text-green-300 mb-2">Check your email</h3>
                            <p className="text-green-700 dark:text-green-400 mb-6">We have sent a password reset code to <strong>{email}</strong></p>
                            <div className="space-y-3">
                                <Link to="/reset-password" state={{ email }} className="block w-full bg-green-600 hover:bg-green-700 text-white py-3 px-4 rounded-xl font-semibold transition-colors shadow-sm">
                                    Enter Reset Code
                                </Link>
                                <button onClick={() => setEmailSent(false)} className="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200">
                        ...
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={loading}
                                ...
                                {loading ? (
                                    ...
                                    <div className="flex items-center justify-center space-x-2">
                                        <span>Send Reset Code</span><Send className="w-4 h-4" />
                                    </div>
                                )}
                            </motion.button>
                        </form>
            ...
            <div className="hidden lg:flex flex-1 bg-[linear-gradient(135deg,_#182233_0%,_#0f172a_100%)]">
                <div className="flex-1 flex items-center justify-center p-12 text-white text-center">
                    <div className="max-w-md">
                        ...
                        <p className="text-primary-100 text-lg">Don't worry, we'll help you get back to your account in a few simple steps.</p>
                    </div>
                </div>
            </div>
        </div>
